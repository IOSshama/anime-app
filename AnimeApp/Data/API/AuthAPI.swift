...
enum AuthAPI {
    static func requestOTP(email: String) throws -> APIEndpoint {
        ...
        return APIEndpoint(path: "/api/auth/otp/request", method: .post, body: body)
    }
    ...
    static func verifyOTP(email: String, code: String) throws -> APIEndpoint {
        ...
        return APIEndpoint(path: "/api/auth/otp/verify", method: .post, body: body)
    }

    static func refresh() -> APIEndpoint {
        APIEndpoint(path: "/api/auth/refresh", method: .post, requiresAuthorization: true)
    }

    static func logout() -> APIEndpoint {
        APIEndpoint(path: "/api/auth/logout", method: .post, requiresAuthorization: true)
    }
}
